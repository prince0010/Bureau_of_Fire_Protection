...
<div class="row">
        ...
        $user = getByID('purpose',checkParamId('id'));
        // If its status == 200 then it means they found a data in the database table, its chgecking every data by ID of that datatable
        if($user['status'] == 200)
        {
            ?>
        <input type="hidden" value =<?=$user['data']['id'];?> name = "purposeId" required>
                        <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label> Purpose </label>
                                <textarea name="purpose" cols="30" rows="10" class ="form-control"><?=$user['data']['purpose'] ?> </textarea>
                            </div>
                            </div>
                            </div>
                            <div class="col-md-6">
                        <div class="mb-3">
                            <button type = "submit" name = "editPurpose" class="btn btn-dark"> Save </button>
                    </div>
                        </div>
                        </div>
                        <?php
